...
#define E_FILE "E_s.txt"
#define R_FILE "R_s.txt"
...
#define TOTAL_COST_LIMIT 1500.0f
...
//            }
    ...
    // sum_j { Z2_{ij} } = 1  forall i (N)
    for (int i = 1; i <= N; i++) {
        glp_set_row_name(lp, row_num, (string("limit-Z2") + itos(i)).c_str());
        glp_set_row_bnds(lp, row_num, GLP_FX, 1, 1);
        for (int j = 1; j <= N; j++) {
            ia[idx] = row_num;
    ...
    string path_cplex = "cplex_files/" + prob_name + ".cplex";
    string path_mps = "mps_files/" + prob_name + ".gz";
    glp_write_lp(lp, NULL, path_cplex.c_str());
    glp_write_mps(lp, GLP_MPS_FILE, NULL, path_mps.c_str());
