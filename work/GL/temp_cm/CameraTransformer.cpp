...
void CameraTransformer::recalibrateCamera(
    ...
    char *images_root = "../../charminar_model_broken_full/dense.nvm.cmvs/00/visualize/";
    ...
    for (int i = 0; i < pts2d_old.size(); i++) {
        pts2d_old[i].x += im_q.cols/2;
        pts2d_old[i].y += im_q.rows/2;
    }

    perspectiveTransform(pts2d_old, pts2d, H);
    
    for (int i = 0; i < pts2d.size(); i++) {
        pts2d[i].x -= im_q.cols/2;
        pts2d[i].y -= im_q.rows/2;
    }
    ...
    if (refineOnFExp) {
        refineOnF(pts3d, pts2d, pts2d_old, im_q.cols/2, im_q.rows/2);
...
void CameraTransformer::visualizeMatching(
    ...
    int i;
    float ppx = new_img.cols/2, ppy = new_img.rows/2;;
...
void CameraTransformer::refineOnF(
        vector<Point3f>& pts3d,
        vector<Point2f>& pts2d_output,  // 0 set
        vector<Point2f>& pts2d,       // set to 
        int col_offset,
        int row_offset) {
    ...
    for (int i = 0; i < seedPts.size(); i++) {
        seedPts2.push_back(Point2f(
                    this->seedPts[i].pt.x - col_offset,
                    this->seedPts[i].pt.y - row_offset));
        qPts2.push_back(Point2f(
                    this->qPts[i].pt.x - col_offset,
                    this->qPts[i].pt.y - row_offset));
    }
